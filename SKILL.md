...
**Level: max** specialized skill (not merged). Category hint: `claude`.

## Summary

Persistent memory layer for Claude Code's /compact command. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `compahook` or user asks by name
...
2. **Confirm tooling** - Verify `compahook` runtime/CLI/MCP is available; document install if missing.
